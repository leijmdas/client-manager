...
<template>
  <div>
    ...
    <v-table ref="table" :page="page" class="panel-main" :table-minheight="450">
      ...
      <el-table-column prop="amount" label="合计(元)" width="80">
        <template slot-scope="{row}">{{row.totalAmount - row.discountAmount - row.refundAmount}}</template>
      </el-table-column>
      ...
      <el-table-column prop="" label="支付方式" width="100"></el-table-column>
    </v-table>
  </div>
</template>
...
export default {
  components:{MerchantSelect,ParkSelect},
  data() {
    return {
      formStatus: 1,
