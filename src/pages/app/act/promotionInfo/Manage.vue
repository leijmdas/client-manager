  ...
  <div>
    ...
    <v-table ref="table" :page="page" :table-minheight="450" @dataloaded="onDataloaded">
      ...
      <el-table-column prop="effectDays" label="有效期" width="80">
        <template slot-scope="{row}">
          {{row.effectTimes !=0 ? row.effectTimes: '不限'}} {{row.effectTimes !=0? $dict.getText(row.effectTimeUnit,$dict.store.PROMOTION_EFFECT_TIMEUNIT): ''}}
        </template>
      </el-table-column>
      ...
      <el-table-column prop="tip" label="总量/发放/使用" width="100">
        <template slot-scope="{row}">
          {{row.limitTotalNum != -1?row.limitTotalNum:'不限'}} / {{ (summaryMap[row.id]||{instanceNum:0}).instanceNum}} /  {{ (summaryMap[row.id]||{usedNum:0}).usedNum}}
        </template>
      </el-table-column>
      ...
      <el-table-column width="100" label="操作" :fixed="'right'">
        <template slot-scope="scope">
            <!-- 主体类型为商户时不暂时任何操作按钮 -->
          <el-button type="text" title="上/下架" @click="toggleStatus(scope.row)" v-if="scope.row.subjectType != 2">
          ...
          <el-button type="text" title="编辑" @click="edit(scope.row)" v-if="scope.row.subjectType != 2">
